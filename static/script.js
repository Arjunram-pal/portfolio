...
function displayBlogs(blogs) {
  ...
  blogs.forEach((blog, index) => {
    ...
    const escapedCategory = escapeHtml(rawCategory);
    const blogId = String(blog.id);
    blogDataById.set(blogId, {
      id: blog.id,
      title: rawTitle,
    ...
    blogsContainer.innerHTML += `
      <li class="blog-post-item">
        <div class="blog-content blog-sequence-card">
          ...
          <div class="blog-meta">
            <p class="blog-category">${escapedCategory}</p>
            <span class="dot"></span>
            <time>${date}</time>
          </div>
          <h3 class="h3 blog-item-title">
            <button class="blog-open-btn" type="button" data-blog-open-id="${blogId}" aria-label="Open full blog ${escapedTitle}">
              ${escapedTitle}
            </button>
          </h3>
          <p class="blog-text">${previewContent}</p>
          <button class="blog-read-btn" type="button" data-blog-open-id="${blogId}" aria-label="Read full blog ${escapedTitle}">
            Read Full
          </button>
          ${adminActions}
        </div>
...
if (blogsContainer) {
  blogsContainer.addEventListener("click", (event) => {
    ...
    if (openTrigger) {
      const blogId = openTrigger.getAttribute("data-blog-open-id");
      if (!blogId) return;
      const blogData = blogDataById.get(blogId);
      if (!blogData) return;
      window.openBlogModal(blogData.id, blogData.title, blogData.category, blogData.date, blogData.content);
      return;
    }

    const editTrigger = event.target.closest("[data-blog-edit-id]");
    if (editTrigger && isAdmin) {
      const blogId = editTrigger.getAttribute("data-blog-edit-id");
      if (!blogId) return;
      const blogData = blogDataById.get(blogId);
      if (!blogData) return;
      editBlog(blogData.id, blogData.title, blogData.category, blogData.content);
      return;
    }

    const deleteTrigger = event.target.closest("[data-blog-delete-id]");
    if (deleteTrigger && isAdmin) {
      const blogId = deleteTrigger.getAttribute("data-blog-delete-id");
      if (!blogId) return;
      const blogData = blogDataById.get(blogId);
      if (!blogData) return;
